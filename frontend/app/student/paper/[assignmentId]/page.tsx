...
export default function StudentPaperPage({ params }: { params: Promise<{ assignmentId: string }> }) {
    ...
    return (
      <div className="flex-1 flex items-center justify-center px-6">
        <div className="text-center">
          ...
          <p className="text-gray-400 text-[14px] mb-6">Your answer sheet has been sent to your teacher.</p>
          <button
    ...
    <div className="flex flex-col h-full bg-transparent overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="bg-white mx-0 md:mx-4 md:my-4 md:rounded-[28px] p-5 md:p-10 shadow-sm">
          <div className="max-w-3xl mx-auto">
            ...
            <div className="flex justify-between text-[14px] text-gray-700 mb-5 font-medium">
              <span>Time: <strong>{paper.timeAllowed}</strong></span>
              <span>Max Marks: <strong>{paper.maximumMarks}</strong></span>
            </div>
            <p className="text-[13px] font-bold text-gray-800 mb-6">All questions are compulsory unless stated otherwise.</p>

            {paper.sections.map((section, si) => (
              <div key={section.id} className="mb-8">
                <h2 className="text-center text-[16px] font-extrabold text-gray-900 mb-3">{section.title}</h2>
                ...
                <ol className="flex flex-col gap-3 text-[14px] text-gray-800">
                  {section.questions.map((q, qi) => (
                    <li key={q.id} className="flex gap-3">
                      <span className="shrink-0 font-semibold text-gray-500 w-5 text-right">{qi + 1}.</span>
                      <span className="leading-relaxed">
                        [{q.difficulty}] {q.text} <span className="text-gray-400 text-[12px]">[{q.marks} marks]</span>
                      </span>
                    </li>
                  ))}
                </ol>
                {si === paper.sections.length - 1 && (
                  <p className="text-[13px] font-extrabold text-gray-800 mt-6">End of Question Paper</p>
                )}
              </div>
            ))}
          </div>
        ...
        <div className="mx-0 md:mx-4 mb-0 md:mb-4 bg-[#2a2a2a] md:rounded-[28px] p-6 md:p-8">
          ...
          <p className="text-gray-400 text-[13px] mb-5">Take a clear photo or scan of your handwritten answers, then upload here.</p>
          ...
          <div
            ...
            className={`border-2 border-dashed rounded-2xl p-6 text-center cursor-pointer transition-colors ${
              dragActive ? "border-orange-400 bg-orange-500/10" : "border-gray-600 hover:border-gray-500"
            }`}
          >
            ...
            {file ? (
              ...
              <>
                ...
                <p className="text-gray-600 text-[11px] mt-1">PDF or image (JPG, PNG, WEBP) · max 20 MB</p>
              </>
            )}
          </div>
          <input
          ...
          {uploadError && (
            <p className="text-red-400 text-[13px] mt-3">{uploadError}</p>
          )}
